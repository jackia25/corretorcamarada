...
export default function EditProperty() {
  ...
  async function fetchProperty() {
    ...
    if (error || !data) {
      toast({
        variant: 'destructive',
        title: 'Erro',
        description: 'Imóvel não encontrado ou você não tem permissão.',
      });
      navigate('/properties');
      return;
    }
    ...
    setSensitivePhotos(data.sensitive_photos || []);
    setLoading(false);
  }
  ...
  return (
    <Layout>
      <div className="container py-8 max-w-3xl">
        <Button variant="ghost" className="mb-4 gap-2" onClick={() => navigate(`/properties/${id}`)}>
          <ArrowLeft className="h-4 w-4" />
          Voltar
        </Button>

        <div className="mb-8">
          <h1 className="text-3xl font-display font-bold">Editar Imóvel</h1>
          <p className="text-muted-foreground">
            Altere os dados do imóvel. Os dados sensíveis só serão compartilhados após acordo de cooperação.
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-8">
          ...
          <div className="flex gap-4">
            ...
            <Button type="submit" className="flex-1 gradient-bg" disabled={saving}>
